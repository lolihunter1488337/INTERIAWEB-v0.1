...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (website) return res.status(200).json({ ok: true, v: VERSION }); // honeypot
    ...
    if (!j.ok) return res.status(502).json({ ok: false, error: j.description || "telegram_error", v: VERSION });
    return res.status(200).json({ ok: true, v: VERSION });
  } catch (e) {
