...
## 📱 PWA + 🎛 ПУЛЬТ АРТИСТОВ (СДЕЛАНО 2026-07-02)
- PWA: панель = устанавливаемое приложение (телефон+ПК). public/manifest.webmanifest (start_url /#/panel, standalone,
  чёрная тема), public/icon-192/512.png (хром-лого на чёрном, генерил jimp), public/sw.js (БЕЗ кэша — чтобы не было
  устаревших сборок), регистрация SW в main.jsx, apple-мета в index.html. Ставится через «Установить приложение».
- Пульт артистов (Фаза 1, вкладка по умолчанию «🎛 Пульт артистов»): ArtistBoard в Panel.jsx, KV ключ interia:artists.
  Колонки: артист · чат-ссылка · ответственный · стадия (8 стадий) · 📄🎵📣 (галочки форм) · дата активности +
  авто «молчит N дней 🔴» (≥5 дн) · заметка. Решает боль «много чатов, теряется статус». Концепт: ПЛАН ЛЕЙБЛА/ПУЛЬТ-АРТИСТОВ.md.
  Фаза 2 (авто-статусы из форм) и Фаза 3 (Telegram-бот: онбординг, алерты, дайджест) — впереди.
- ⚠️ jimp закреплён на 0.22.12 (1.x ломает API make-og/make-icons скриптов).

## 🔐 АВТОРИЗАЦИЯ (СДЕЛАНО 2026-07-02) — БД пользователей
- api/_users.js — 9 пользователей (login/name/nick + SHA256-хеш пароля; ПЛАЙНТЕКСТ паролей НЕ в репо).
  Пароли для раздачи: C:\Users\1\Desktop\INTERIA RECORDS\ДОСТУПЫ-ПАНЕЛЬ.txt (НЕ в гите).
- api/auth.js — POST {login,password} → {ok,name}. Панель: вход по логину+паролю, показ имени, кнопка «Выйти».
- /api/panel и /api/ya теперь требуют валидного юзера (x-panel-key = пароль, сервер хеширует и сверяет с БД).
  ⇒ ДЫРА ОТКРЫТОГО API ЗАКРЫТА (проверено: без ключа 401, с валидным 200). PANEL_PASSWORD больше не нужен.
- TODO апгрейд: bcrypt вместо sha256, httpOnly-cookie сессии вместо пароля в sessionStorage (когда дойдут руки).

## 🔒 БЕЗОПАСНОСТЬ v2 (аудит 2026-07-02, установлены 820 cyber-скиллов в ~/.claude/skills)
> ПОЛНЫЙ ОТЧЁТ + ЧЕК-ЛИСТ ФИКСОВ: ПЛАН ЛЕЙБЛА/БЕЗОПАСНОСТЬ.md. ✅ п.1 (открытый API) ЗАКРЫТ авторизацией.
> Кратко находки: внешняя поверхность чистая; главный риск (открытый API) — ЗАКРЫТ.
...
- Вёдра INTERIA CONTROL (CONTROL[]): attention/overdue/ready/docs/cover/pitch/week — счётчики + клик=фильтр.
- Карточка артиста (раскрывается): чат/контакт/дата/заметка + таблица треков с пайплайном (форма→обложка→отгрузка→питч→вышел).
- Вкладка «Сегодня» (Today) агрегирует: соцсети-пост дня, задачи, артисты-внимание (по новой модели).
- Формы Google (для Фазы B): ссылки в ПЛАН ЛЕЙБЛА/ПУЛЬТ-АРТИСТОВ.md. Доступа к ответам пока нет → B ждёт.
- Дальше: B формы→авто-галочки (Apps Script→/api/form), C Яндекс→«вышел», D бот+AI-сводки чатов+авто-напоминания.

## 📝 TODO / ИДЕИ (2026-07-02, партия 2)
- BELIEVE: юзеру УЖЕ ДАЛИ доступ к кабинету. Нужен план: узнать, что даёт Backstage (экспорт CSV/Excel по релизам и стримам? API?)
  → тянуть цифры (стримы/доход/статус релиза) в Пульт/Релизы. Пока не знаем возможности кабинета — спросить/посмотреть.
- ИДЕЯ: брендовые HTML-письма (email) — красиво оформленные, с лого INTERIA (шаблоны рассылок артистам/партнёрам).
- ВОПРОС: зачем вкладка «Библия» — юзер не видит цели (была идея GPT). Решить: репозиционировать как база стандартов/онбординга
  ИЛИ интегрировать активные части (release checklist, pitch-гайд) в рабочий процесс, ИЛИ убрать. Обсуждается.
- СДЕЛАНО в эту сессию: вкладка «🏠 Сегодня» (агрегатор дня), «Релизы» = авто-каталог с Яндекса (labels/6401624, read-only,
  ничего вручную), Дашборд переименован из «Планы» + обновлён под Label OS, запоминание активной вкладки (localStorage interia_panel_tab).
...
- 🐞 БАГ (в работе): в «Задачах» появляются задачи, которые уже удаляли. Причина — рассинхрон общего хранилища
  (устаревший инстанс/вкладка перезаписывает сервер last-write-wins). Фикс: refresh при фокусе окна + dirty-guard в useShared.
